...
End-to-end automation for RHOAI Conforma exception requests: validates inputs,
creates required Jira tickets (RHOAIENG + PSX/OCPEXCEPT), generates the
exception YAML, creates a GitLab MR in `releng/konflux-release-data`, and
cross-links all artifacts.
...
curl -fsSL https://raw.githubusercontent.com/opendatahub-io/aiops-infra/conforma-exception-create-ai-skill/skills/conforma-exception-create/install.sh | bash -s -- --project /path/to/my-project
```

### Uninstall

```bash
curl -fsSL https://raw.githubusercontent.com/opendatahub-io/aiops-infra/conforma-exception-create-ai-skill/skills/conforma-exception-create/install.sh | bash -s -- --uninstall
```
...
> Install the conforma-exception-create skill from
...
|---|---|
| Claude Code (global) | `~/.claude/skills/conforma-exception-create/` |
| Cursor (global) | `~/.cursor/skills-cursor/conforma-exception-create/` |
| Project-local | `<project>/.claude/skills/conforma-exception-create/` |
...
```bash
curl -fsSL https://raw.githubusercontent.com/opendatahub-io/aiops-infra/conforma-exception-create-ai-skill/skills/conforma-exception-create/install.sh | bash -s -- --branch feature-branch
```
