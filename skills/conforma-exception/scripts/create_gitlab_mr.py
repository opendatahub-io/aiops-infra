...
def _build_mr_title_consolidated(
    rule: str, version_specs: list[dict], vendor_tag: str | None = None
) -> str:
...
def _update_effective_until_in_content(
    content: str, line_idx: int, new_effective_until: str
) -> str:
...
def apply_exception_to_policy_file(
        ...
        if exc["has_component_names"]:
            if sorted(exc["component_names"]) == sorted_components:
                if exc["effective_until_line"] is not None:
                    content = _update_effective_until_in_content(
                        content, exc["effective_until_line"], effective_until
                    )
                    file_path.write_text(content, encoding="utf-8")
    ...
    return {
        "action": "appended",
        "detail": (
            "Existing exception found for this rule but with different componentNames; "
            "appended new block"
        ),
    }
    ...
    try:
        ...
        _run_git(
            ["git", "clone", "--depth=1", "--branch", DEFAULT_BRANCH,
             "--filter=blob:none", "--sparse", repo_url, clone_dest],
            timeout=300,
        )
        repo_dir = workdir / "repo"
        policy_dir = str(Path(target_file).parent)
        ...
        mr_body = _build_mr_body_consolidated(
            ...
            rhoaieng_url=rhoaieng_url, reference_url=reference_url,
        ...
        mr_create_args = [
            ...
            "--remove-source-branch",
            "-R", f"{GITLAB_HOST}/{GITLAB_PROJECT}",
        ]
        if use_fork:
            mr_create_args.extend(["--head", branch_name])
...
def remove_expired_mr(
    ...
    try:
        ...
        result = remove_exception_from_policy_file(
            file_path, rule, effective_until, components,
        )
...
def modernize_exception_mr(
    ...
    try:
        ...
        remove_result = remove_exception_from_policy_file(
            pf, rule, old_effective_until, old_components,
        )
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--version-specs-json", default=None,
        help=(
...
def main() -> int:
    ...
    if args.modernize_expired_exception:
        if not args.rule or not args.effective_until:
            print(
                "Error: --rule and --effective-until are required "
                "for --modernize-expired-exception mode.",
                file=sys.stderr,
            )
            return 1
