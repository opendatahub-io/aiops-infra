...
```
~/.conforma/bin/conforma_run.sh scripts/conforma_policy_ops.py check-gate --rule <rule> --components <comp1,comp2> --policy-files <file1,file2> [--clone-dir ~/.conforma/konflux-release-data] [--environment prod]
```

## Examples

```bash
~/.conforma/bin/conforma_run.sh scripts/conforma_policy_ops.py check-gate \
