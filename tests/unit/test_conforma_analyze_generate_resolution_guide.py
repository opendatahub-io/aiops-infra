...
class TestFullyCoveredViolation:
    ...
    def test_permanent_exception_label(self, tmp_path, sample_violations_yaml, sample_catalog):
        data = {
...
class TestMetadataAiModelFooter:
    def test_includes_ai_model_footer(self):
        ...
        assert "*Generated by: aiops-infra conforma-analyze skill (LLM: claude-sonnet-4-5)*" in header
...
class TestRenderWorkScope:
    ...
    def test_high_cardinality_shows_csv_link(self):
        ...
        render_work_scope(lines, "sbom_spdx.disallowed_package_attributes", work_scope_by_rule, "https://csv-url")
        assert len(lines) == 2
...
class TestComponentStem:
    ...
    def test_stem_equality_across_versions(self):
        ...
        assert mod._component_stem("odh-workbench-jupyter-minimal-cpu-py312-v3-4") == mod._component_stem(
...
class TestRenderComponentsTable:
    ...
    def test_mr_cell_populated_by_stem_match(self):
        v = self._base_violation(
            ...
            exception_details_by_component=[
                {
                    "component": "odh-vllm-cpu-v3-5-ea-2",
                    "file": None,
                    "line": None,
...
class TestMainAutoExtraction:
    ...
    def test_auto_extracts_source_from_metadata_file(
        ...
        rc = self._run_main(
            [
                "--violations-yaml",
                str(sample_violations_yaml),
                "--coverage-json",
                str(sample_coverage_json),
                "--reports-dir",
                str(tmp_path),
                "--catalog",
                str(sample_catalog),
...
class TestUpcomingReleaseDate:
    ...
    def test_expiring_before_release_appears_before_uncovered(self, tmp_path, sample_violations_yaml, sample_catalog):
        data = {
    ...
    def test_expiring_with_mr_sufficient_expiry(self, tmp_path, sample_violations_yaml, sample_catalog):
    ...
    def test_expiring_with_mr_insufficient_expiry(self, tmp_path, sample_violations_yaml, sample_catalog):
        ...
        assert (
            "| Effective Until in Existing Exception | Exception Effective Until in Open Merge Request | Merge Request |"
            in content
        )
        assert "2026-07-15" in content
        assert "[!19385]" in content

    def test_expiring_tiers_ordering(self, tmp_path, sample_violations_yaml, sample_catalog):
            ...
            "violations": [
                {
                    ...
                    "exception_details_by_component": [
                        {
                            "component": "comp-no-mr",
                            "effective_until": "2026-07-01",
                            "file": "p.yaml",
                            "line": 1,
                            "url": "https://example.com/p.yaml#L1",
                        },
                    ],
                    "open_merge_requests": [],
                    "open_jira_tickets": [],
                ...
                },
                {
                    ...
                    "exception_details_by_component": [
                        {
                            "component": "comp-insuf-mr",
                            "effective_until": "2026-07-01",
                            "file": "p.yaml",
                            "line": 2,
                            "url": "https://example.com/p.yaml#L2",
                        },
                    ],
                    "open_merge_requests": [
                        {
                            "iid": 100,
                            "url": "https://example.com/-/merge_requests/100",
                            "mr_type": "exception",
                            "mr_components": ["comp-insuf-mr"],
                ...
                },
                {
                    ...
                    "exception_details_by_component": [
                        {
                            "component": "comp-suf-mr",
                            "effective_until": "2026-07-01",
                            "file": "p.yaml",
                            "line": 3,
                            "url": "https://example.com/p.yaml#L3",
                        },
                    ],
                    "open_merge_requests": [
                        {
                            "iid": 200,
                            "url": "https://example.com/-/merge_requests/200",
                            "mr_type": "exception",
                            "mr_components": ["comp-suf-mr"],
    ...
    def test_always_shows_zero_count_headers(self, tmp_path, sample_violations_yaml, sample_catalog):
...
class TestViolationLinks:
    ...
    def test_violation_titles_are_anchor_links(
        ...
        for rule in ["hermetic_task.hermetic", "sbom_spdx.disallowed_package_attributes"]:
            ...
            assert link in todo_section, f"Expected anchor link {link} in TODO section"
...
class TestCoverageSummaryPolicyFileLinks:
    ...
    def test_coverage_line_includes_file_links(
        self,
        tmp_path,
        sample_violations_yaml,
        sample_catalog,
    ):
        policy_files = [
            {"name": "registry-rhoai-stage.yaml", "url": "https://gitlab.example.com/policy/registry-rhoai-stage.yaml"},
            {
                "name": "exceptions/fbc-rhoai-stage.yaml",
                "url": "https://gitlab.example.com/exceptions/fbc-rhoai-stage.yaml",
            },
        ]
        data = {
            "summary": {
...
class TestContextIntegration:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                ["--release", "rhoai-3.5-ea.2", "--violations-yaml", "v.yaml", "--coverage-json", "c.json"],
                "--reports-dir",
            ),
            (
                [
    ...
    def test_main_uses_context_analysis_and_tooling_outputs(self, tmp_path, monkeypatch, sample_catalog):
        run_dir = self._setup_run_with_artifacts(tmp_path, monkeypatch, sample_catalog)
        (run_dir / "conforma-analysis.md").write_text("analysis")
        (run_dir / "tooling-health.json").write_text(json.dumps({"tools": []}))
        conforma_context_ops.update_step(run_dir, "tooling_health", "completed", health_json="tooling-health.json")
        (run_dir / "fetch-metadata.json").write_text(
            json.dumps(
                {
                    "releases": {
                        "rhoai-3.5-ea.2": {
    ...
    def test_main_reports_generation_file_error(
        ...
        monkeypatch.setattr(
            mod, "generate_resolution_guide", lambda **_: (_ for _ in ()).throw(FileNotFoundError("missing"))
        )
        monkeypatch.setattr(
            "sys.argv",
            [
...
def _covered_violation(rule, components, expiry_details=None, open_mrs=None, is_permanent=False, earliest_expiry=None):
...
class TestComputeViolationBuckets:
    ...
    def test_buckets_mr21322_exact_scenario_two_components_wildcard(self):
        ...
        mr = _mr(21322, "https://example.com/mr/21322", ["*"], effective_until="2026-08-12")
        coverage = _make_coverage_data(
            violations=[
    ...
    def test_buckets_invalid_upcoming_release_date_all_ok(self):
        ...
        buckets = _compute_violation_buckets(
            ...
            result,
            by_cr,
            upcoming_release_date="invalid-date",
        )
        assert len(buckets["has_mr_expires_before_release"]) == 0
        assert len(buckets["has_mr_ok"]) == 1
...
class TestKeyTakeawaysToolingTodo:
    ...
    def test_healthy_todo_0_includes_tooling_health_table(self):
        ...
        assert (
            "| conforma-reporter | HEALTHY | [#2](https://gh/run/2) -- success (2026-08-06) | 0 | [#1](https://gh/run/1) (2026-08-05) |"
            in output
        )
...
class TestKeyTakeawaysAnchors:
    ...
    def test_table_anchors_with_expiring(self):
        ...
        output = render_key_takeaways(
            ...
            result,
            by_cr,
            upcoming_release_date="2026-08-01",
        )
        assert "### TODO #1" in output
        assert "### TODO #2" in output
        assert "### DONE #" in output
...
class TestGuideSourceCsvRowsAndAiModel:
    ...
    def _generate(self, tmp_path, sample_violations_yaml, sample_coverage_json, sample_catalog, **kwargs):
    ...
    def test_source_csv_rows_override(self, tmp_path, sample_violations_yaml, sample_coverage_json, sample_catalog):
    ...
    def test_ai_model_footer_in_guide(self, tmp_path, sample_violations_yaml, sample_coverage_json, sample_catalog):
        content = self._generate(
            tmp_path,
            sample_violations_yaml,
            sample_coverage_json,
        ...
        assert "*Generated by: aiops-infra conforma-analyze skill (LLM: claude-sonnet-4-5)*" in content
    ...
    def test_ai_model_cli_flag(
        ...
        assert mod.main() == 0
        content = output_file.read_text()
        assert "*Generated by: aiops-infra conforma-analyze skill (LLM: test-model-123)*" in content
    ...
    def test_ai_model_from_context_yaml(
        ...
        assert mod.main() == 0
        content = output_file.read_text()
        assert "*Generated by: aiops-infra conforma-analyze skill (LLM: context-model-456)*" in content
...
class TestRenderJiraTickets:
    ...
    def test_related_search_link_rendered_in_create_cell(self):
        ...
        refs, create_url, search_url, label = _sync_component_cells(sync["violations"][0], "comp-a-v3-5-ea-2")
        assert refs == []
        ...
        assert (
            "[Search related Jira (label: `conforma-rhoai-3-6-ea-2-comp-a-hermetic-task-hermetic`)](https://jira/issues/?jql=related)"
            in _build_jira_cell([], [], refs, create_url, search_url, label)
        )
...
class TestRenderResolutionGuideJira:
    ...
    def test_jira_sync_with_created_renders_block(self, coverage_with_one_violation: dict, sample_catalog: Path):
        catalog = mod._load_catalog(sample_catalog)
        sync = _jira_sync_with_created("hermetic_task.hermetic", ["comp-a-v3-5-ea-2"], "RHOAIENG-1", "https://jira/1")
    ...
    def test_jira_sync_unrelated_rule_is_noop(self, coverage_with_one_violation: dict, sample_catalog: Path):
        catalog = mod._load_catalog(sample_catalog)
...
class TestGenerateResolutionGuideWithJiraSync:
    ...
    def test_absent_jira_sync_is_fallback(self, tmp_path, sample_violations_yaml, sample_coverage_json, sample_catalog):
        content = self._generate(tmp_path, sample_violations_yaml, sample_coverage_json, sample_catalog)
    ...
    def test_jira_sync_renders_block(self, tmp_path, sample_violations_yaml, sample_coverage_json, sample_catalog):
...
class TestGuideRendererBranchCoverage:
    ...
    def test_bucket_date_variants(self):
        ...
        invalid_expiry = _covered_violation(
            "rule.d",
            ["comp-e"],
            earliest_expiry="not-a-date",
        )
        result = _make_analysis_result(total_violations=3)
        buckets = _compute_violation_buckets(
            _make_coverage_data([covered, uncovered_mr, no_date, invalid_expiry]),
            result,
            {
                ("rule.a", "comp-a"): 1,
                ("rule.a", "comp-b"): 1,
    ...
    def test_warning_and_resolution_renderers_cover_optional_fields(self):
        ...
        render_key_takeaways(_make_coverage_data(), result, {}, upcoming_release_date="2026-09-01")
    ...
    def test_components_and_false_alerts_cover_slack_and_exception_variants(self, sample_catalog):
            ...
            "open_merge_requests": [
                {
                    "iid": 1,
                    "url": "https://gitlab/1",
                    "mr_components": ["rhoai-fbc-fragment-v3-5"],
        ...
        render_known_false_alerts(
            false_alert_lines, "test.no_failed_tests", violation, mod._load_catalog(sample_catalog)
        )
