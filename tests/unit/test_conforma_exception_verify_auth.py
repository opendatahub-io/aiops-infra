...
class TestCheckAcliAvailable:
    def test_native_available(self):
        with (
            patch.object(verify_auth, "_find_acli", return_value="/usr/bin/acli"),
    ...
    def test_container_available(self):
        with (
            patch.object(verify_auth, "_find_acli", return_value=None),
...
class TestCheckAcliAuth:
    ...
    def test_not_authenticated(self):
        ...
        with (
            patch.object(verify_auth, "run_acli", return_value=mock_result),
    ...
    def test_timeout(self):
        with (
            patch.object(verify_auth, "run_acli", side_effect=subprocess.TimeoutExpired("acli", 15)),
...
class TestCheckGlabAuth:
    ...
    def test_not_logged_in(self):
        ...
        with (
            patch.object(verify_auth, "run_glab", return_value=mock_result),
    ...
    def test_timeout(self):
        with (
            patch.object(verify_auth, "run_glab", side_effect=subprocess.TimeoutExpired("glab", 15)),
...
class TestRunChecks:
    ...
    def test_stops_acli_auth_when_unavailable(self):
        with (
            patch.object(
                verify_auth, "check_acli_available", return_value={"check": "acli_available", "passed": False}
            ),
            patch.object(verify_auth, "check_glab_available", return_value={"check": "glab_available", "passed": True}),
            patch.object(verify_auth, "check_glab_auth", return_value={"check": "glab_auth", "passed": True}),
            patch.object(
    ...
    def test_stops_glab_auth_when_unavailable(self):
        with (
            ...
            patch.object(verify_auth, "_setup_jira_rest_api"),
            patch.object(
                verify_auth, "check_glab_available", return_value={"check": "glab_available", "passed": False}
            ),
            patch.object(
                verify_auth, "_check_jira_library_auth", return_value={"check": "jira_library_auth", "passed": True}
            ),
