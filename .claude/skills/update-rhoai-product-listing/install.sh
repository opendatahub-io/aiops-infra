#!/usr/bin/env bash
# install.sh — Install the update-rhoai-product-listing Claude Code skill
#
# Usage:
...
SKILL_NAME="update-rhoai-product-listing"
...
if [[ -z "${PYXIS_REPO_CONFIGS_REPO_URL:-}" ]]; then
  warn "PYXIS_REPO_CONFIGS_REPO_URL is not set — will default to:"
  warn "  https://gitlab.cee.redhat.com/releng/pyxis-repo-configs.git"
else
  success "PYXIS_REPO_CONFIGS_REPO_URL=${PYXIS_REPO_CONFIGS_REPO_URL}"
fi
...
if [[ -z "${GITLAB_SSL_VERIFY:-}" ]]; then
  warn "GITLAB_SSL_VERIFY is not set — will default to true"
  warn "  Set to 'false' if you encounter SSL certificate errors with gitlab.cee.redhat.com"
else
  success "GITLAB_SSL_VERIFY=${GITLAB_SSL_VERIFY}"
fi
...
if ! $CREDS_OK; then
  ...
  echo "    # export PYXIS_REPO_CONFIGS_REPO_URL='https://gitlab.cee.redhat.com/releng/pyxis-repo-configs.git'"
  echo "    # export JIRA_SERVER='https://redhat.atlassian.net'"
  echo "    # export GITLAB_SSL_VERIFY='false'  # if you hit SSL errors"
  echo ""
...
echo ""
echo "    /update-rhoai-product-listing https://redhat.atlassian.net/browse/RHOAIENG-1234"
echo ""
echo "  NOTE: This skill requires:"
echo "    - Red Hat VPN to reach gitlab.cee.redhat.com"
echo "    - GITLAB_TOKEN with 'api' and 'write_repository' scopes"
