#!/usr/bin/env bash
# install.sh — Install the add-rhoai-dockerfile-labels Claude Code skill
#
# Usage:
...
SKILL_NAME="add-rhoai-dockerfile-labels"
...
# curl — needed for the GitHub API label check in Step 5 of the skill
if ! command -v curl &>/dev/null; then
...
check_var "GITHUB_TOKEN"    "Needs 'repo' scope (read and write, push access to component repo)" "true"
...
echo ""
echo "    /add-rhoai-dockerfile-labels https://redhat.atlassian.net/browse/RHOAIENG-1234"
echo ""
echo "  NOTE: This skill requires:"
echo "    - Public internet access to github.com (no VPN needed)"
echo "    - GITHUB_TOKEN with 'repo' scope and push access to the component repo"
