#!/usr/bin/env bash
# install.sh — Install the integrate-component-with-bundle Claude Code skill
#
# Usage:
...
SKILL_NAME="integrate-component-with-bundle"
...
check_var "GITHUB_TOKEN"    "Needs 'repo' scope with push access to the OBC repo" "true"
...
if [[ -z "${OBC_REPO_URL:-}" ]]; then
  warn "OBC_REPO_URL is not set — will default to:"
  warn "  https://github.com/opendatahub-io/ODH-Build-Config.git"
else
  success "OBC_REPO_URL=${OBC_REPO_URL}"
fi
...
if ! $CREDS_OK; then
  ...
  echo "    # export OBC_REPO_URL='https://github.com/opendatahub-io/ODH-Build-Config.git'"
...
echo ""
echo "    /integrate-component-with-bundle https://redhat.atlassian.net/browse/RHODS-14226"
echo ""
echo "  NOTE: This skill requires:"
echo "    - Public internet access to github.com"
echo "    - GITHUB_TOKEN with push access to the ODH-Build-Config repo (or OBC_REPO_URL override)"
echo "    - 'component_onboarding_details.yaml' attached to the Jira issue"
echo "    - Both bundle/bundle-patch.yaml and config/build-config.yaml must exist in the repo"
echo ""
