#!/usr/bin/env bash
# install.sh — Install the setup-auto-merge Claude Code skill
#
# Usage:
...
# curl — needed for GitHub API calls in Steps 3f and 4
if ! command -v curl &>/dev/null; then
...
check_var "GITHUB_TOKEN"    "Needs 'repo' scope + push access to rhods-devops-infra" "true"
check_var "JIRA_USER_EMAIL" "export JIRA_USER_EMAIL=you@redhat.com (required when using Jira URL)"
...
if [[ -z "${RHODS_DEVOPS_INFRA_REPO_URL:-}" ]]; then
  warn "RHODS_DEVOPS_INFRA_REPO_URL is not set — will default to:"
  warn "  https://github.com/red-hat-data-services/rhods-devops-infra.git"
else
  success "RHODS_DEVOPS_INFRA_REPO_URL=${RHODS_DEVOPS_INFRA_REPO_URL}"
fi
...
if ! $CREDS_OK; then
  ...
  echo "    export GITHUB_TOKEN='your-github-token'   # needs: repo scope + push to rhods-devops-infra"
  echo "    export JIRA_USER_EMAIL='you@redhat.com'   # only needed when passing a Jira URL"
  echo "    export JIRA_API_TOKEN='your-jira-api-token'"
  echo ""
  echo "    # Optional overrides:"
  echo "    # export RHODS_DEVOPS_INFRA_REPO_URL='https://github.com/red-hat-data-services/rhods-devops-infra.git'"
...
echo ""
echo "    /setup-auto-merge https://redhat.atlassian.net/browse/RHOAIENG-1234"
echo ""
echo "  Or without a Jira issue:"
echo ""
echo "    /setup-auto-merge"
echo ""
echo "  NOTE: This skill requires:"
echo "    - Public internet access to github.com (no VPN needed)"
echo "    - GITHUB_TOKEN with 'repo' scope and push access to rhods-devops-infra"
