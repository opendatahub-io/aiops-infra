...
Stop with the error message on failure; do not continue.
...
`init_pipeline.sh` creates `$WORKDIR`, writes the initial `pipeline_state.json` skeleton
(all steps at "pending"), or prints current step statuses if resuming. It outputs
`WORKDIR=...` and `JIRA_ID=...` assignments consumed by `eval`.
...
`parse_component_details.sh` reads `component_onboarding_details.yaml`, extracts
`COMPONENT_NAME`, `IS_OPERATOR`, `REPO_URL`, `REPO_BRANCH`, derives `PRODUCT_CONTEXT`
(from Jira key prefix or summary field), derives Quay variables, updates `pipeline_state.json`,
and marks non-applicable steps as "skipped". Outputs shell variable assignments consumed by `eval`.

If `PRODUCT_CONTEXT == "UNKNOWN"`, ask the user: "Is this component for ODH or RHOAI?"
Then manually update `product_context` in `pipeline_state.json` and re-run the script.
...
4. Launch the deferred renovate sync script:
   ```bash
...
Launch the deferred workflow script:
```bash
...
`raise_jira_review.sh` reads `pipeline_state.json`, builds the step summary table,
and calls `update_jira_issue.py` to add the `onboarding-in-review` label, set status
to "Review", and post the comment.
...
`monitor_completion.sh` polls `pipeline_state.json` every 2 minutes (max 4 hours),
syncs `monitor_*.result` files into state, and when all applicable steps are done,
transitions Jira to "Resolved" with a full summary comment.
...
| Deferred workflow 422 error | 14 deferred | OKC PR not yet merged; script waits automatically |
| Deferred workflow times out (3h) | 14 deferred | Check `deferred_workflow.log`; re-run script manually |
| Tekton PR not in workflow logs | 14 deferred | Check run URL in Jira; update `pipeline_state.json` manually |
...
| Completion monitor times out (4h) | 16 | Check `.result` files; re-run `monitor_completion.sh` manually |
