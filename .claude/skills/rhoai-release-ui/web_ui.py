...
COMMON_SCRIPTS = SKILL_DIR.parent / "common" / "scripts"
...
async def _run_pipeline(job_id: str, req: RunRequest) -> None:
    ...
    steps_cfg = [
        (
            ...
            ["uv", "run", "--script", str(COMMON_SCRIPTS / "run_rbc_release.py"),
             req.previous_version, req.new_version] + _dry,
        ),
        (
            "RBC Main Onboard",
            "Main-branch catalog + Tekton file onboarding — GitHub",
            ["uv", "run", "--script", str(COMMON_SCRIPTS / "run_rbc_main.py"),
             req.previous_version, req.new_version] + _dry,
        ),
        (
            "Konflux Onboard",
            "Clone, edit, commit, push and open MR — GitLab konflux-release-data",
            ["uv", "run", "--script", str(COMMON_SCRIPTS / "run_konflux_onboard.py"),
             req.previous_version, req.new_version, "--repo-dir", req.repo_dir] + _dry,
        ),
    ]
...
if __name__ == "__main__":
    import uvicorn
    print("Starting RHOAI Release Onboarding Web UI...")
