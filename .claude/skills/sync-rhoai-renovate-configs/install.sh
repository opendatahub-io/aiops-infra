#!/usr/bin/env bash
# install.sh — Install the sync-rhoai-renovate-configs Claude Code skill
#
# Usage:
...
SKILL_NAME="sync-rhoai-renovate-configs"
...
check_var "GITHUB_TOKEN"    "Needs 'repo' + 'actions:write' scope (or 'workflow' scope)" "true"
...
if ! $CREDS_OK; then
  ...
  echo "    export GITHUB_TOKEN='your-github-token'   # needs: repo + actions:write (or workflow)"
...
echo ""
echo "    /sync-rhoai-renovate-configs https://redhat.atlassian.net/browse/RHOAIENG-1234"
echo ""
echo "  Or without a Jira issue:"
echo ""
echo "    /sync-rhoai-renovate-configs"
echo ""
echo "  NOTE: This skill requires:"
echo "    - Public internet access to github.com (no VPN needed)"
echo "    - GITHUB_TOKEN with 'repo' + 'actions:write' scope"
echo "      (triggers the sync-renovate-configs.yml workflow in rhoai-konflux-central)"
echo ""
